...
export class AppointmentService {
  ...
  getAll(page: number = 1, limit: number = 10): Observable<PaginatedResponse<Appointment>> {
    return this.httpService.getList<Appointment>(
  ...
  getByDoctor(doctorId: string): Observable<PaginatedResponse<Appointment>> {
    return this.httpService.getList<Appointment>(
      APPOINTMENT_ENDPOINTS.GET_BY_DOCTOR(doctorId)
    );
  }

 //oluştur
  create(appointment: CreateAppointmentRequest): Observable<ApiResponse<Appointment>> {
    return this.httpService.post<Appointment>(
  ...
  update(id: string, appointment: CreateAppointmentRequest): Observable<ApiResponse<Appointment>> {
    return this.httpService.put<Appointment>(
  ...
  patch(id: string, appointment: Partial<CreateAppointmentRequest>): Observable<ApiResponse<Appointment>> {
    return this.httpService.patch<Appointment>(
      APPOINTMENT_ENDPOINTS.UPDATE(id),
      appointment
    );
  }
